...
class AcquisitionEI(AcquisitionBase):
    ...
    def acquisition_function(self, x):
        ...
        return -f_acqu  # Return negative value for posterior minimization
...
class AcquisitionMP(AcquisitionBase):
    ...
    def update_batches(self, X_batch, L, Min):
        ...
        if X_batch is not None:
            self.r_x0, self.s_x0 = self._hammer_function_precompute(X_batch,
                                                                    L,
                                                                    Min,
                                                                    self.model)
    ...
    def _hammer_function(self, x, x0, r_x0, s_x0):
        ...
        '''
        return norm.logcdf(
            (np.sqrt((np.square(np.atleast_2d(x)[:, None, :] -
