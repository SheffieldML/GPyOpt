...
class AcquisitionMPI_PoF(AcquisitionBase):
    """
    ...
    def __init__(self, model, model_c, space, optimizer=None, cost_withGradients=None, jitter=0.01, jitter_c=None, void_min = 1e5):
        ...
        super(AcquisitionMPI_PoF, self).__init__(model, space, optimizer, cost_withGradients=cost_withGradients)
        self.jitter = jitter
    ...
    def fromConfig(model, space, optimizer, cost_withGradients, config):
        return AcquisitionMPI_PoF(model, space, optimizer, cost_withGradients, jitter=config['jitter'])

    def _compute_acq(self, x):
        """
        Computes the Constrained Maximum probability of improvement per unit of cost
        """
        ########################################################################
        pY, _  = self.model.predict(self.model.model.X)
        pC = np.zeros((pY.shape[0],len(self.model_c)))
        for ic in range(len(self.model_c)):
            pC[:,ic] = self.model_c[ic].model.Y[:,0] #((self.model_c[ic]).predict(self.model.model.X))[0][:,0]
    ...
    def _compute_acq_withGradients(self, x):
        """
        Computes the Constrained Maximum probability of improvement and its derivative (has a very easy derivative!)
        """
        ########################################################################
        pY, _  = self.model.predict(self.model.model.X)
        pC = np.zeros((pY.shape[0],len(self.model_c)))
